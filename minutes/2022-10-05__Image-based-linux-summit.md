// Also see https://md.archlinux.org/features
...
1. Provide feedback to systemd-syscfg: https://github.com/systemd/systemd/issues/24864 
...
- Documentation on pre-calculated and signed PCR values:
   - https://github.com/systemd/systemd/blob/main/man/systemd-measure.xml
 (Unrendered, updated soon)
   - https://www.freedesktop.org/software/systemd/man/systemd-measure.html (may be outdated)
   - https://github.com/systemd/systemd/blob/main/man/systemd-stub.xml
...
- Discoverable Partition Specification: https://systemd.io/DISCOVERABLE_PARTITIONS/
   - PRs to extend this please file to https://github.com/systemd/systemd/blob/main/docs/DISCOVERABLE_PARTITIONS.md  
...
- for building UKIs, DDIs, sysext, initrd, OS images (w/ split partition support for individual partition update images)
   ...
   - Native support on OBS - https://build.opensuse.org/project/show/home:bluca:mkosi 
...
- There’s osbuild: https://www.osbuild.org/
...
- Flatcar uses custom python script inherited from CoreOS, always builds images from sources (Gentoo style)
   - Adds SLSA provenance (sources fingerprints -> binaries fingerprints) to OS image during build. See https://www.flatcar.org/docs/latest/reference/supply-chain/  for details.
...
- OpenSuSE uses the [libeconf](https://opensuse.github.io/libeconf/) project
...
- system-repart: declarative configuration of system partitions, can populate systems with files too, cryptographically secure
...
- Automatic boot assessment after update to mark partition/disk/whatever as good/bad
   ...
   - GnomeOS nudge to use boot-complete: https://gitlab.gnome.org/GNOME/gnome-build-meta/-/issues/560
...
- Support for fallback in Shim/grub
   - https://github.com/rhboot/shim/pull/502
...
- Full reset vs. selective to keep wanted local data
   ...
   - **TODO #12** has a sub-item to add support for removing files to systemd-tempfiles to better support partial reset
...
- Ubuntu core supports reset via recovery partition (image file in ESP), see https://ubuntu.com/core/docs/use-recovery-mode, 
...
- Flatcar: initial provisioning is full image, via cloud store / marketplace or manual download from image server (https). Image contains full partition layout; root partition is extended during provisioning to fill whole disk
   ...
   - Updates are orchestrated using the Omaha protocol, (Chrome OS update protocol)
      ...
      - Implemented in Nebraska server, is FOSS: https://github.com/kinvolk/nebraska 
...
- Systemd-sysupdate
   ...
   - can use casync as transport, which chunks an image (like zsync) into files (unlike zsync). Hence zsync needs http range requests server-side, casync does not. See https://github.com/systemd/casync, http://zsync.moria.org.uk/
